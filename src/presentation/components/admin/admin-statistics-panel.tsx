...
function DistributionBars({ rows, barClass }: { rows: CountRow[]; barClass?: (label: string) => string }) {
  if (rows.length === 0) {
    return <p className="text-sm text-slate-500">Sin datos para mostrar.</p>;
  }
        ...
        <li key={row.label}>
          <div className="mb-1 flex items-center justify-between gap-2 text-sm">
            ...
            <span className="shrink-0 text-slate-500">
              {row.count} <span className="text-slate-400">({row.percent}%)</span>
            </span>
          </div>
    ...
    <Card title="Estadísticas del sistema">
        ...
        <div className="rounded-2xl border border-violet-100 bg-violet-50/50 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-extrabold text-slate-900">{analytics.uniqueUsers}</p>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div>
          ...
          <DistributionBars rows={analytics.byGameType} barClass={() => "bg-brand-primary"} />
        </div>
      </div>
    </Card>
    </div>
